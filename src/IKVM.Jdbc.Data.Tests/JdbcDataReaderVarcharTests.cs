...
namespace IKVM.Jdbc.Data.Tests
{
    ...
    [TestClass]
    public class JdbcDataReaderVarcharTests
    {

        static JdbcDataReaderVarcharTests()
        {
        ...
        public void CanGetString()
        {
            ...
            cmd.CommandText = "SELECT CAST('THIS IS A STRING' AS VARCHAR)";
        ...
        public void CanGetCharsToArray()
        {
            ...
            using var cmd = cnn.CreateCommand();
            cmd.CommandText = "SELECT CAST('AAAAAAAAAAAAAAAA' AS VARCHAR)";
            using var rdr = cmd.ExecuteReader();
            Assert.IsTrue(rdr.Read());
            Assert.AreEqual(1, rdr.FieldCount);
        ...
        public void CanGetGuid()
        {
            ...
            using var cmd = cnn.CreateCommand();
            cmd.CommandText = "SELECT CAST(CAST('75281326-D2E0-497D-AB70-5C3183E22A38' AS UUID) AS VARCHAR)";
            using var rdr = cmd.ExecuteReader();
            Assert.IsTrue(rdr.Read());
            Assert.AreEqual(1, rdr.FieldCount);
